...
static void odbc_rows_func(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    if (argc != 2) {
        sqlite3_result_error(ctx, "bo_query_rows requires 2 arguments", -1);
        return;
    }
    if (sqlite3_value_type(argv[0]) == SQLITE_NULL ||
        sqlite3_value_type(argv[1]) == SQLITE_NULL) {
    ...
    const char *conn_str = (const char *)sqlite3_value_text(argv[0]);
    const char *query    = (const char *)sqlite3_value_text(argv[1]);
    char *result = blobodbc_query_rows(conn_str, query);
    if (result) {
        sqlite3_result_text(ctx, result, -1, SQLITE_TRANSIENT);
        blobodbc_free(result);
    } else {
...
/* ── bo_query_rows_named(conn_str, sql, bind_json) ─────────────────── */

static void odbc_rows_named_func(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    if (argc != 3) {
        sqlite3_result_error(ctx, "bo_query_rows_named requires 3 arguments", -1);
        return;
    }
    if (sqlite3_value_type(argv[0]) == SQLITE_NULL ||
        sqlite3_value_type(argv[1]) == SQLITE_NULL) {
    ...
    char *result = blobodbc_query_rows_named(conn_str, query, bind_json);
    if (result) {
        sqlite3_result_text(ctx, result, -1, SQLITE_TRANSIENT);
        blobodbc_free(result);
    } else {
...
int sqlite3_blobodbc_init(sqlite3 *db, char **pzErrMsg,
    ...
    rc = sqlite3_create_function(db, "bo_query_rows", 2,
                                  SQLITE_UTF8, NULL, odbc_rows_func, NULL, NULL);
    if (rc != SQLITE_OK) return rc;

    rc = sqlite3_create_function(db, "bo_query_rows_named", 3,
                                  SQLITE_UTF8, NULL, odbc_rows_named_func, NULL, NULL);
    if (rc != SQLITE_OK) return rc;
